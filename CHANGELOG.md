...
Changes specific to this fork rather than upstream. See the
[README](./README.md) for what the fork is and [docs/](./docs/) for detail.

**Breaking**

- Studio's Logs Explorer and Reports tabs no longer work. They query Logflare
  over `/analytics/v1` in Logflare's own SQL dialect, which Grafana does not
  provide. `NEXT_PUBLIC_ENABLE_LOGS` is `false` so the tabs are off rather than
  broken. See [docs/observability.md](./docs/observability.md)
- The `/pg/` route to postgres-meta is removed. It performs no authentication of
  its own and its `/query` endpoint runs arbitrary SQL as a privileged role.
  Studio reaches it directly over the compose network. See
  [docs/gateway.md](./docs/gateway.md)
- The gateway now checks that an `apikey` or `Authorization` header is present
  rather than comparing it to the real key. Each service still validates the JWT
  itself. Requests with an invalid key now reach the service instead of being
  rejected at the gateway
- `docker compose up` requires the external `observability` network:
  `docker network create observability`

**Gateway**

- Fixed CORS preflight being rejected. Router rules matched on literal key
  values, which a preflight does not carry, so cross-origin browser clients
  failed before their real request went out
- Removed the `service_role` key from gateway config. Traefik's `/api/rawdata`
  rendered router rules verbatim and was unauthenticated on the compose network
- Turned off the Traefik API and dashboard (`api.insecure`)
- Removed the boot-time entrypoint that ran `apk add apache2-utils` on every
  start and ran the gateway as root. The basic-auth file is generated by
  `utils/generate-htpasswd.sh` using openssl
- Scoped the Studio catch-all away from API namespaces. Keyless API requests
  were answering `WWW-Authenticate: Basic`, popping a browser login dialog
- Split dynamic config into `volumes/api/dynamic/` fragments. A directory mount
  also makes `watch: true` work, which a single-file bind mount silently broke
- Router priorities are now explicit; Traefik defaults to rule-string length

**Auth**

- Studio can run behind OIDC via oauth2-proxy, under the `oidc` compose profile.
  See [docs/oidc.md](./docs/oidc.md)

**Observability**

- Vector ships to Loki instead of Logflare, keeping all per-service parsing.
  Logs from Studio, meta, imgproxy, Supavisor and oauth2-proxy are now shipped
  rather than dropped
- Prometheus metrics enabled on Traefik, PostgREST, GoTrue, imgproxy and Storage
- Added a `postgres-exporter` sidecar
- Traefik's access log is JSON

**Removed**

- `analytics` (Logflare) and all eight `depends_on` blocks referencing it

**Repository**

- Added `.env.example`, which `reset.sh` expected but which never existed
- Added `docs/`
